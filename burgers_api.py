...
def create_user(user_data):
    user_response = requests.post(urls.BASE_URL+urls.CREATE_USER, json=user_data)
...
def delete_user(access_token):
    ...
    response_delete = requests.delete(urls.BASE_URL + urls.GET_USER, headers=headers)
    return response_delete

@allure.step("Авторизовать зарегистрированного пользователя")
def login_user(login_data):
    login_response = requests.post(urls.BASE_URL + urls.LOGIN_USER, json=login_data)
    return login_response

@allure.step("Изменить данные пользователя: email, password, name")
def change_user_data(access_token, new_data):
    headers = {"Authorization": access_token}
    change_data_response = requests.patch(urls.BASE_URL + urls.GET_USER, headers=headers, json=new_data)
...
def get_ingredients():
    response = requests.get(urls.BASE_URL + urls.GET_INGREDIENTS)
    return response

@allure.step("Создать заказ")
def create_new_order(headers, ingredients):
    response = requests.post(urls.BASE_URL + urls.GET_USER_ORDERS, headers=headers, json=ingredients)
    return response

@allure.step("Получить все заказы конкретного пользователя")
def get_users_orders(headers):
    response = requests.get(urls.BASE_URL + urls.GET_USER_ORDERS, headers=headers)
    return response